        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.ziyuanneirong" label="资源内容" prop="ziyuanneirong">
                    <span v-html="ruleForm.ziyuanneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.leixingOptions = "原生资源,外部资源,网络资源".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `xuekeziyuan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.ziyuanneirong = this.ruleForm.ziyuanneirong.replace(reg,'../../../springbooto97z9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xuekeziyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xuekeziyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
