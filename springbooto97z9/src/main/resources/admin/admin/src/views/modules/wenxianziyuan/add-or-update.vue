<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="文献类型" prop="wenxianleixing">
          <el-select v-model="ruleForm.wenxianleixing" placeholder="请选择文献类型">
            <el-option
                v-for="(item,index) in wenxianleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="文献类型" prop="wenxianleixing">
	      <el-input v-model="ruleForm.wenxianleixing"
                placeholder="文献类型" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="文献内容" prop="wenxianneirong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.wenxianneirong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.wenxianneirong" label="文献内容" prop="wenxianneirong">
                    <span v-html="ruleForm.wenxianneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	wenxianleixing : false,
	wenxianneirong : false,
	fengmian : false,
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.ziyuanmingcheng = true;
          ...
          if(o=='wenxianleixing'){
            this.ruleForm.wenxianleixing = obj[o];
	    this.ro.wenxianleixing = true;
            continue;
          }
          if(o=='wenxianneirong'){
            this.ruleForm.wenxianneirong = obj[o];
	    this.ro.wenxianneirong = true;
            continue;
          }
          if(o=='fengmian'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.wenxianleixingOptions = "学科教材,学术期刊,学位论文".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `wenxianziyuan/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.wenxianneirong = this.ruleForm.wenxianneirong.replace(reg,'../../../springbooto97z9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wenxianziyuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wenxianziyuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
