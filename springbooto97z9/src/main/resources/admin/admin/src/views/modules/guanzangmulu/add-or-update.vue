<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="目录编号" prop="mulubianhao">
            <el-input v-model="ruleForm.mulubianhao" 
                placeholder="目录编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.mulubianhao" label="目录编号" prop="mulubianhao">
              <el-input v-model="ruleForm.mulubianhao" 
                placeholder="目录编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="目录名称" prop="mulumingcheng">
          <el-input v-model="ruleForm.mulumingcheng" 
              placeholder="目录名称" clearable  :readonly="ro.mulumingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="目录名称" prop="mulumingcheng">
              <el-input v-model="ruleForm.mulumingcheng" 
                placeholder="目录名称" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='mulubianhao'){
            this.ruleForm.mulubianhao = obj[o];
	    this.ro.mulubianhao = true;
            continue;
          }
          if(o=='mulumingcheng'){
            this.ruleForm.mulumingcheng = obj[o];
	    this.ro.mulumingcheng = true;
            continue;
          }
          if(o=='neirong'){
	...
	this.ruleForm.neirong = this.ruleForm.neirong.replace(reg,'../../../springbooto97z9/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `guanzangmulu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.guanzangmuluCrossAddOrUpdateFlag = false;
                  this.parent.search();
